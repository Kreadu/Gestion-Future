...
// .wrangler/tmp/bundle-qnKSZm/checked-fetch.js
...
// .wrangler/tmp/bundle-qnKSZm/strip-cf-connecting-ip-header.js
...
var ColombiaPayrollEngine = class {
  /**
   * Determina la tarifa progresiva del Fondo de Solidaridad Pensional (FSP)
   * con base en los SMMLV equivalentes del IBC.
   */
  ...
  static calculate(input) {
    ...
    const extraDiurnaHours = input.extraDiurna ?? input.overtimeHours?.extraDiurna ?? 0;
    const extraNocturnaHours = input.extraNocturna ?? input.overtimeHours?.extraNocturna ?? 0;
    const recargoNocturnoHours = input.recargoNocturno ?? input.overtimeHours?.recargoNocturno ?? 0;
    ...
    const health8_5pct = isExempt ? 0 : ibcSecuritySocial * 0.085;
    const pension12pct = ibcSecuritySocial * 0.12;
    const arlValue = ibcSecuritySocial * arlRate;
    const sena2pct = isExempt ? 0 : ibcSecuritySocial * 0.02;
    const icbf3pct = isExempt ? 0 : ibcSecuritySocial * 0.03;
    const ccf4pct = ibcSecuritySocial * 0.04;
    const totalContributions = health8_5pct + pension12pct + arlValue + sena2pct + icbf3pct + ccf4pct;
    const cesantias = grossEarnings * (1 / 12);
    const interesesCesantias = cesantias * 0.01;
    const primaServicios = grossEarnings * (1 / 12);
    const vacaciones = ibcSecuritySocial * (1 / 24);
    const totalProvisions = cesantias + interesesCesantias + primaServicios + vacaciones;
    const round = /* @__PURE__ */ __name((val) => Math.round(val * 100) / 100, "round");
    return {
...
// .wrangler/tmp/bundle-qnKSZm/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-qnKSZm/middleware-loader.entry.ts
